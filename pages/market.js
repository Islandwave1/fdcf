...
export default function Market(){
  ...
  useEffect(()=>{
    const saved = JSON.parse(localStorage.getItem('iw_market')||'[]')
    setItems(saved)
  },[])
  ...
  async function addItem(){
    ...
    setItems(next); localStorage.setItem('iw_market', JSON.stringify(next))
    setF({title:'',price:'',desc:'',email:'',image:''})
  }
  ...
  return (<Layout>
    ...
    <div className="card mb-4">
      <div className="grid md:grid-cols-2 gap-3">
        ...
        <input className="input md:col-span-2" type="file" accept="image/*" onChange={async (e)=>{
          const file=e.target.files?.[0]; if(file){ const b64=await toBase64(file); setF({...f,image:b64}) }
        }}/>
      </div>
    ...
    <div className="grid md:grid-cols-3 gap-4">
      {items.map(it=>(<div key={it.id} className="card">
        ...
        <div className="text-white/80">${it.price}</div>
        <div className="text-white/80 text-sm mt-1">{it.desc}</div>
        {it.email && <a className="btn btn-alt mt-2" href={`mailto:${it.email}?subject=Marketplace: ${encodeURIComponent(it.title)}`}>Contact Seller</a>}
      </div>))}
    </div>
  </Layout>)
}
